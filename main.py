...
#shows all
@app.get("/books/auther/{book_auther}",response_model=list[schema.BookRespose])
def show_with_auther(book_auther:str,db:Session = Depends(get_db)):
    db_book_by_auther = db.query(model.Book).filter(model.Book.auther_name==book_auther).all()
...
def show_all_with_journal(book_journal:str,db:Session = Depends(get_db)):
    db_book_by_journal = db.query(model.Book).filter(model.Book.journal == book_journal).all()
